...
import TabelDataPosKebawetan from "@/components/common/tabel/tabelPosHujanKepahiang/TabelDataPosKebawetan";
...
import { tambahDataPosHujanExcel } from "@/lib/api/rain-gauge/rain-gauge-insert-excel/router";
    ...
    const handleSave = async () => {
        try {
            ...
            if (inputType === "manual") {
                ...
                const result = await tambahDataPosHujanExcel(
                    user_id,
                    "Kepahiang",
                    "Kabawetan",
                    fileBase64
                ...
                if (result?.success) {
                    toast.success(
                        "Data pos hujan dari Excel berhasil ditambahkan"
                    );
                    setIsOpenModal(false);
                    resetForm();
                } else {
                    toast.error("Gagal menyimpan data dari file Excel");
                }
            }
        } catch (error: unknown) {
                        ...
                        {inputType === "otomatis" && (
                            <div>
                                ...
                                <p className="text-xs text-black mt-2">
                                    ...
                                    . Data petir akan diproses otomatis
                                    berdasarkan file yang diupload.
                                </p>
                            </div>
                        )}
                    </div>
