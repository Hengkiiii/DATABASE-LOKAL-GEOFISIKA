...
export default function GempaPage() {
  ...
  const handleManualSubmit = async () => {
    try {
      ...
      const [hh = "00", mm = "00", ss = "00"] = dateTime.time.split(":");
      const formattedDateTime = `${dateTime.date}T${hh}:${mm}:${ss}+00:00`;
      ...
      toast.error(
        error instanceof Error ? error.message : "Gagal menambahkan data gempa"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleExcelSubmit = async () => {
    try {
      ...
      if (result?.success) {
        ...
        toast.error(result?.message || "Gagal menambahkan data dari file");
      }
    } catch (error: unknown) {
          ...
          {inputType === "manual" && (
            <>
              ...
              <input
                ...
                onChange={(e) => {
                  const [date, timeFull] = e.target.value.split("T");
              ...
              />
              <p className="text-xs text-gray-500 mt-2">*Field wajib diisi</p>
            </>
          )}
        </div>
        <div
...
2023-09-02T08:15:00+00:00 -7.456 110.456 15 4.8 II"
              ...
              <p className="text-xs text-black mt-2">
                Format:{" "}
                <strong>
                  date_time(YYYY-MM-DDTHH:MM:SS+00:00) Lintang Bujur Kedalaman
                  Magnitude MMI
                </strong>
                <br />
