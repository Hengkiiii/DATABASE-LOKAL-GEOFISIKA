...
export default function TekananUdaraPage() {
  ...
  const handleManualSubmit = async () => {
    try {
      ...
      const totalAirPressure = (tekanan07Num + tekanan13Num + tekanan18Num) / 3;
  ...
  const handleExcelSubmit = async () => {
    try {
      ...
      console.error("Gagal menambahkan data tekanan udara dari file:", error);
          ...
          >
            <div>
              ...
              <div className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-md shadow-sm bg-white text-gray-700 hover:bg-gray-100 transition">
                ...
                <label className="cursor-pointer">
                  <span className="truncate max-w-[150px]" title={file?.name}>
                    {file ? file.name : "Upload File (.csv / .xlsx)"}
                  </span>
                  <input
                    type="file"
                    accept=".csv,.xlsx"
              ...
              <p className="text-xs text-black mt-2">
                ...
                <strong className="text-red-800">Jam 07:00</strong>,{" "}
                <strong className="text-red-800">Jam 13:00</strong>,{" "}
                <strong className="text-red-800">Jam 18:00</strong>, dan{" "}
                <strong className="text-red-800">tanggal</strong>. Tekanan udara
                akan dihitung otomatis berdasarkan waktu pengamatan harian.
              </p>
            </div>
          </div>
