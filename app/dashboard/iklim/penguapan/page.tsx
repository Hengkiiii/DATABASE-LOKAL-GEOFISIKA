  ...
  const handleManualSubmit = async () => {
    try {
      ...
      if (inputType === "manual") {
        ...
        if (!penguapan) {
          toast.warning("Penguapan harus diisi");
          return;
        }

        // Konversi penguapan ke number agar sesuai tipe API
        const evaporationValue = Number(penguapan);
        if (isNaN(evaporationValue)) {
          toast.warning("Nilai penguapan harus berupa angka");
          return;
        }
      ...
      toast.error(
        (error as Error).message || "Gagal menambahkan data penguapan"
      );
    }
  };
