...
export default function IntensitasHujanPage() {
  ...
  const handleManualSubmit = async () => {
    try {
      ...
      const response = await insertRainIntensity({
        user_id,
        name,
        date: tanggal,
      ...
      toast.error(
        (error as Error).message || "Gagal menambahkan data intensitas hujan"
      );
    }
  };
  ...
  const handleSubmitOtomatis = async () => {
    try {
      ...
      const response = await tambahDataRainIntensityExcel(user_id, fileBase64);
      toast.success("Data intensitas hujan dari file berhasil ditambahkan");
      setIsOpenModal(false);
      resetForm();
      handleReload();
    } catch (error) {
            ...
            {inputType === "otomatis" && (
              <div>
                ...
                <p className="text-xs text-black mt-2">
                  ...
                  <strong className="text-red-800">Nama Stasiun</strong> dan{" "}
                  <strong className="text-red-800">Tanggal</strong>. Nilai
                  intensitas hujan akan dihitung secara otomatis berdasarkan
                  data curah hujan per tanggal.
                </p>
              </div>
            )}
          </div>
