...
export async function getMinTemperatureByDate(
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}min-temperature/get-by-date?start_date=${startDate}&end_date=${endDate}`,
      {
        method: "GET",
        credentials: "include",
      }
    ...
    throw new Error(
      (error as Error).message ||
        "Terjadi kesalahan saat mengambil data suhu udara minimum berdasarkan rentang tanggal"
    );
  }
}
