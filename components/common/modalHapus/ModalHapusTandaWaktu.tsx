...
interface ModalHapusTandaWaktuProps {
  show: boolean;
  data: { id: number; tanggal: string } | null;
  onClose: () => void;
  onDelete: () => void;
  isDarkMode: boolean;
}

export default function ModalHapusTandaWaktu({
  show,
  data,
  onClose,
  onDelete,
  isDarkMode,
}: ModalHapusTandaWaktuProps) {
  return (
    <AnimatePresence>
      {show && data && (
        <motion.div
        ...
        >
          ...
          >
            <h3 className="text-lg font-semibold">Hapus Data Tanda Waktu</h3>
            <p className="text-sm">
              Apakah Anda yakin ingin menghapus data tanggal{" "}
              <strong>{data.tanggal}</strong> dengan nama{" "}
            </p>
