...
interface ModalHapusTekananUdaraProps {
  show: boolean;
  data: { tanggal: string; tekananUdara: number } | null;
  onClose: () => void;
  onDelete: () => void;
  isDarkMode: boolean;
}

export default function ModalHapusTekananUdara({
  show,
  data,
  onClose,
  onDelete,
  isDarkMode,
}: ModalHapusTekananUdaraProps) {
  return (
    <AnimatePresence>
      {show && data && (
        <motion.div
        ...
        >
          ...
          >
            <h3 className="text-lg font-semibold">Hapus Data Tekanan Udara</h3>
            <p className="text-sm">
              Apakah Anda yakin ingin menghapus data tanggal{" "}
              <span className="font-semibold">{data.tanggal}</span> dengan
              tekanan udara{" "}
              <span className="font-semibold">{data.tekananUdara}</span>?
            </p>
