...
interface ModalHapusSuhuUdaraMaksimumProps {
  show: boolean;
  data: { date: string; max_temperature: string } | null;
  onClose: () => void;
  onDelete: () => void;
  isDarkMode: boolean;
}

export default function ModalHapusSuhuUdaraMaksimum({
  show,
  data,
  onClose,
  onDelete,
  isDarkMode,
}: ModalHapusSuhuUdaraMaksimumProps) {
  return (
    <AnimatePresence>
      {show && data && (
        <motion.div
        ...
        >
          ...
          >
            ...
            <p className="text-sm">
              ...
              <strong>{data.date}</strong> dengan suhu maksimum{" "}
              <strong>{data.max_temperature}°C</strong>?
            </p>
