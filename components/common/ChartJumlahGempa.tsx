  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (sortedYears.length > 0 && !sortedYears.includes(tahunDipilih)) {
        ...
        earthquakes.forEach((quake) => {
          try {
            const date = new Date(`${quake.date_time}`);
            if (!isNaN(date.getTime()) && date.getFullYear() === tahunDipilih) {
            ...
            console.warn("Error processing earthquake data:", quake);
          }
        });
    ...
    <Card style="bg-white p-4 space-y-4">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data}>
          ...
          <Tooltip
            formatter={(value: number) => [`${value} gempa`, "Jumlah"]}
