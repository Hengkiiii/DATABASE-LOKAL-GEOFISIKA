...
interface TabelSuhuUdaraMinimum {
  id: number;
  date: string; // Menggunakan string untuk tanggal
  min_temperature: string;
}

interface ModalEditSuhuUdaraMinimumProps {
  show: boolean;
  data: TabelSuhuUdaraMinimum | null;
  onClose: () => void;
  onSave: (data: TabelSuhuUdaraMinimum) => void;
  isDarkMode: boolean;
  setData: React.Dispatch<React.SetStateAction<TabelSuhuUdaraMinimum | null>>;
}

export default function ModalEditSuhuUdaraMinimum({
  show,
  data,
  onClose,
  onSave,
  isDarkMode,
  setData,
}: ModalEditSuhuUdaraMinimumProps) {
  return (
    <AnimatePresence>
      {show && data && (
        <motion.div
              ...
              <InputField
                ...
                onChange={(e) => {
                  ...
                  if (/^-?\d*\.?\d*$/.test(e.target.value)) {
                    setData({ ...data, min_temperature: e.target.value });
                  }
                }}
                darkMode={isDarkMode}
              />
