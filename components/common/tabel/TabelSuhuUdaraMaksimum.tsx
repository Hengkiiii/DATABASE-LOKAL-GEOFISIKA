...
import ModalEditSuhuUdaraMaksimum from "@/components/common/modalEdit/ModalEditSuhuUdaraMaksimum";
import ModalHapusSuhuUdaraMaksimum from "@/components/common/modalHapus/ModalHapusSuhuUdaraMaksimum";
import { getMaxTemperature } from "@/lib/api/max-temperature/max-temperature-get/router";
import { updateMaxTemperature } from "@/lib/api/max-temperature/max-temperature-update/router";
import { deleteMaxTemperature } from "@/lib/api/max-temperature/max-temperature-delete/router";
import { getMaxTemperatureAll } from "@/lib/api/max-temperature/max-temperature-get-all/router";
import { getMaxTemperatureByDate } from "@/lib/api/max-temperature/max-temperature-get-by-date/router";
...
export default function TabelSuhuUdaraMaksimum({
  reload,
}: TabelSuhuUdaraMaksimumProps) {
  ...
  const [dataSuhuUdaraMaksimum, setDataSuhuUdaraMaksimum] = useState<
    TabelSuhuUdaraMaksimum[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [showEditModal, setShowEditModal] = useState(false);
  const [selectedData, setSelectedData] =
    useState<TabelSuhuUdaraMaksimum | null>(null);
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      setDataSuhuUdaraMaksimum(mapped);
      sessionStorage.setItem("maxTemperatureData", JSON.stringify(mapped));
      setCurrentPage(1);
    } catch (error) {
  ...
  const totalPages = Math.ceil(dataSuhuUdaraMaksimum.length / itemsPerPage);
  const paginatedData = dataSuhuUdaraMaksimum.slice(
  ...
  const generatePDF = async () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text("Laporan Data Suhu Udara Maksimum", 105, 48, { align: "center" });
    ...
    const tableData = dataSuhuUdaraMaksimum.map((item, index) => [
      index + 1,
      formatDate(item.date),
      item.max_temperature + "°C",
    ]);

    autoTable(doc, {
      head: [["No.", "Tanggal", "Suhu Udara Maksimum (°C)"]],
      body: tableData,
      startY: 68,
    ...
    const fileName =
      startDate && endDate
        ? `Laporan_Suhu_Udara_Maksimum_${startDate}_${endDate}.pdf`
        : "Laporan_Suhu_Udara_Maksimum_Semua_Data.pdf";

    doc.save(fileName);
  };
  const exportToExcel = () => {
    const worksheetData = dataSuhuUdaraMaksimum.map((item, index) => ({
      No: index + 1,
      Tanggal: formatDate(item.date),
      "Suhu Udara Maksimum (°C)": item.max_temperature,
    }));

    const worksheet = XLSX.utils.json_to_sheet(worksheetData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Data Suhu Maksimum");

    const fileName =
      startDate && endDate
        ? `Laporan_Suhu_Udara_Maksimum_${startDate}_${endDate}.xlsx`
        : "Laporan_Suhu_Udara_Maksimum_Semua_Data.xlsx";
  ...
  const handleEditClick = async (id: number) => {
    try {
      const item = await getMaxTemperature(id);
      const mapped = {
        id: item.id,
        date: item.date,
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      setLoading(true);
      await updateMaxTemperature(selectedData.id.toString(), user_id, {
        date: selectedData.date,
        max_temperature: selectedData.max_temperature,
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await deleteMaxTemperature(dataToDelete.id.toString(), user_id);
      toast.success("Data suhu udara maksimum berhasil dihapus!");
                ...
                <div className="flex justify-between gap-2 pt-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setStartDate("");
                      setEndDate("");
                      sessionStorage.removeItem("maxTemperatureData");
                      fetchAllData();
                      setShowFilter(false);
                    }}
          ...
          <table className="w-full text-sm text-gray-700 border-collapse rounded-lg overflow-hidden">
            <thead className="bg-gray-100 text-gray-600">
              <tr>
                ...
                <th className="py-3 px-5 text-center">Suhu Udara Maksimum</th>
