...
export default function TabelPenguapan({ reload }: TabelPenguapanProps) {
  ...
  const generatePDF = async () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text("Laporan Data Penguapan", 105, 48, { align: "center" });
  ...
  const exportToExcel = () => {
    const worksheetData = dataPenguapan.map((item, index) => ({
      No: index + 1,
      Tanggal: item.tanggal,
      "Penguapan (mm)": item.penguapan,
    }));

    const worksheet = XLSX.utils.json_to_sheet(worksheetData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Data Penguapan");

    const fileName =
      startDate && endDate
        ? `Laporan_Penguapan_${startDate}_${endDate}.xlsx`
        : "Laporan_Penguapan_Semua_Data.xlsx";
