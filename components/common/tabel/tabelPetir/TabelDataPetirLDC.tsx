...
export default function TabelDataPetirLDC() {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await getDataPetirByLightningData("Petir Ldc");
  ...
  const handleEditClick = async (item: LightningData) => {
    try {
      ...
      const detail = await getDataPetirById(item.id.toString(), "Petir Ldc");
  ...
  const handleSaveEdit = async () => {
    ...
    console.log("handleSaveEdit: mulai update data", {
      ...
      dataUpdate: {
        ...
        lightning_data: selectedData.lightning_data || "Petir Ldc",
      },
    });
  ...
  return (
    <Card style="relative bg-white p-6 md:p-8 space-y-6 shadow-xl rounded-2xl">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Data Petir - LDC</h2>
