...
export default function TabelDataPetirCSV() {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await getDataPetirByLightningData("petir Csv");
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      if (response && response.success && Array.isArray(response.data)) {
        setData(response.data);
        sessionStorage.setItem("dataPetirCsv", JSON.stringify(response.data));
      } else {
        setData([]);
  ...
  const handleEditClick = async (item: LightningData) => {
    try {
      ...
      const detail = await getDataPetirById(item.id.toString(), "petir Csv");
  ...
  const handleSaveEdit = async () => {
    ...
    console.log("handleSaveEdit: mulai update data", {
      ...
      dataUpdate: {
        ...
        lightning_data: selectedData.lightning_data || "petir Csv",
      },
    });
              ...
              <div className="flex justify-between gap-2 pt-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    setStartDate("");
                    setEndDate("");
                    sessionStorage.removeItem("dataPetirCsv");
                    fetchAllData();
                    setShowFilter(false);
                  }}
