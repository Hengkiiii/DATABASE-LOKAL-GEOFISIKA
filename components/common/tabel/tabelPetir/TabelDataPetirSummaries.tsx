...
export default function TabelDataPetirSRF() {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await getDataPetirByLightningData("Summaries");
  ...
  const handleEditClick = async (item: LightningData) => {
    try {
      ...
      const detail = await getDataPetirById(item.id.toString(), "Summaries");
  ...
  const handleSaveEdit = async () => {
    ...
    console.log("handleSaveEdit: mulai update data", {
      ...
      dataUpdate: {
        ...
        lightning_data: selectedData.lightning_data || "Summaries",
      },
    });
