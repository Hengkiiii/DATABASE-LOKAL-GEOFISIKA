...
export default function TabelDataPetirSRF() {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await getDataPetirByLightningData("Petir SRF");
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      if (response && response.success && Array.isArray(response.data)) {
        setData(response.data);
        sessionStorage.setItem("dataPetirSRF", JSON.stringify(response.data));
      } else {
        setData([]);
  ...
  const handleEditClick = async (item: LightningData) => {
    try {
      ...
      const detail = await getDataPetirById(item.id.toString(), "Petir SRF");
  ...
  const handleSaveEdit = async () => {
    ...
    console.log("handleSaveEdit: mulai update data", {
      ...
      dataUpdate: {
        ...
        lightning_data: selectedData.lightning_data || "Petir SRF",
      },
    });
  ...
  return (
    <Card style="relative bg-white p-6 md:p-8 space-y-6 shadow-xl rounded-2xl">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Data Petir - SRF</h2>
              ...
              <div className="flex justify-between gap-2 pt-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    setStartDate("");
                    setEndDate("");
                    sessionStorage.removeItem("dataPetirSRF");
                    fetchAllData();
                    setShowFilter(false);
                  }}
