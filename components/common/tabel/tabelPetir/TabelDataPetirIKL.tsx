...
export default function TabelDataPetirIKL() {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await getDataPetirByLightningData("petir Ikl");
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      if (response && response.success && Array.isArray(response.data)) {
        setData(response.data);
        sessionStorage.setItem("dataPetirIkl", JSON.stringify(response.data));
      } else {
        setData([]);
  ...
  const handleEditClick = async (item: LightningData) => {
    try {
      ...
      const detail = await getDataPetirById(item.id.toString(), "petir Ikl");
  ...
  const handleSaveEdit = async () => {
    ...
    console.log("handleSaveEdit: mulai update data", {
      ...
      dataUpdate: {
        ...
        lightning_data: selectedData.lightning_data || "petir Ikl",
      },
    });
  ...
  return (
    <Card style="relative bg-white p-6 md:p-8 space-y-6 shadow-xl rounded-2xl">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Data Petir - IKL</h2>
              ...
              <div className="flex justify-between gap-2 pt-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    setStartDate("");
                    setEndDate("");
                    sessionStorage.removeItem("dataPetirIkl");
                    fetchAllData();
                    setShowFilter(false);
                  }}
