...
import { editAdmin } from "@/lib/api/admin/admin-edit/router"; // import fungsi editAdmin
...
export default function DaftarAdmin() {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      const message =
        (err as Error).message || "Gagal menghapus admin. Silakan coba lagi.";
      toast.error(message);
    } finally {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const message = (err as Error).message || "Gagal memuat data admin";
        setError(message);
        toast.error(message);
      } finally {
        setLoading(false);
  ...
  return (
    <>
      <Card style="bg-white p-6 md:p-8 space-y-6 shadow-xl rounded-2xl mt-6 ml-6 mr-6">
        <h2 className="text-xl font-bold text-gray-800">
          ...
          {loginAdmin && (
            <span className="block text-sm text-gray-500 mt-1">
              Login sebagai: {loginAdmin.firstName} {loginAdmin.lastName}
            </span>
          )}
        </h2>
        ...
        {error && (
          <p className="text-center text-black">Data tidak ditemukan.</p>
        )}
            ...
            <table className="w-full text-sm text-gray-700 border-collapse rounded-lg overflow-hidden">
              <thead className="bg-gray-100 text-gray-600">
                <tr>
                  ...
                  <th className="py-3 px-5 text-left">Nama Lengkap</th>
              ...
              <tbody>
                {admins.length > 0 ? (
                  admins
                    .filter((admin) => admin.id !== loginAdmin?.id)
                    .map((admin, index) => (
                      <tr
                        key={admin.id}
                        className={`${
                      ...
                      >
                        ...
                        <td className="py-3 px-5 capitalize">{admin.role}</td>
                        <td className="py-3 px-5">
                          <div className="flex justify-center gap-3">
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                const adminLoginData = await getAdmin(
                                  admin.userId
                                );
                                setEditAdminData(adminLoginData);
