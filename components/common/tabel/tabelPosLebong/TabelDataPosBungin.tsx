...
export default function TabelDataPosKebawetan() {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await getDataPosHujanByCityVillage("Lebong", "Bungin");
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      if (response && response.success && Array.isArray(response.data)) {
        setData(response.data);
        sessionStorage.setItem("dataPosBungin", JSON.stringify(response.data));
      } else {
        setData([]);
  ...
  const handleSaveEdit = async () => {
    ...
    console.log("handleSaveEdit: mulai update data", {
      ...
      dataUpdate: {
        ...
        city: selectedData.city || "Lebong",
        village: selectedData.village || "Bungin",
      },
    });
              ...
              <div className="flex justify-between gap-2 pt-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    setStartDate("");
                    setEndDate("");
                    sessionStorage.removeItem("dataPosBungin");
                    fetchAllData();
                    setShowFilter(false);
                  }}
