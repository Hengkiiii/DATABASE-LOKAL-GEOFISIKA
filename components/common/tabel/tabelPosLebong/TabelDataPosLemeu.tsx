...
export default function TabelDataPosKebawetan() {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await getDataPosHujanByCityVillage("Lebong", "Lemeu");
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      if (response && response.success && Array.isArray(response.data)) {
        setData(response.data);
        sessionStorage.setItem("dataPosLemeu", JSON.stringify(response.data));
      } else {
        setData([]);
  ...
  const handleSaveEdit = async () => {
    ...
    console.log("handleSaveEdit: mulai update data", {
      ...
      dataUpdate: {
        ...
        village: selectedData.village || "Lemeu",
      },
    });
              ...
              <div className="flex justify-between gap-2 pt-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    setStartDate("");
                    setEndDate("");
                    sessionStorage.removeItem("dataPosLemeu");
                    fetchAllData();
                    setShowFilter(false);
                  }}
